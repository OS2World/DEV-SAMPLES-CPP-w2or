/* RC language:
 ...
 block              ::=  [ dialog-block | menu-block | stringtable-block | bitmap-block | icon-block | rcdata-block | cursor-block | accelarator-block | rcinclude-block]
 ...
 rcinclude-block    ::= 'rcinclude' <filename>
*/
...
void doBlock() {
        ...
        //'rcinclude' is also special
        if(tc==tc_literal && stricmp(buf,"RCINCLUDE")==0) {
                doRCINCLUDE();
                return;
        }
